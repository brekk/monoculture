...
In order to add comments that `monodoc` can pick up, the comments must start with `/**` (note the two asterisks), end with `*/` (as all multiline comments must) and have a leading `*` per line.
...
This is then consumed by `monodoc` downstream, and it will spit out a JSON file with a lot of entries that look like:
...
To add documentation which `monodoc` can understand and auto generate, (including a live example), you'd want to do something like this:
...
`yarn workspace docs run autodoc` should be all that is required when you change any file in any workspace (unless it's within the `monodoc` itself) to add magic / `jsdoc` style comments. But if something is broken, see the [troubleshooting](#troubleshooting) section below.

Additionally, `monodoc` generates a file in `apps/docs` which is `monodoc-generated.json`, in the even of an error it might be a useful diagnostic to see what `monodoc` parsed relative to what you wrote.
...
This is a very useful page for [understanding MDX errors](https://mdxjs.com/docs/troubleshooting-mdx/#problems-writing-mdx). In general if you have a class of errors here, your best bet is to play around with the `.mdx` file that `monodoc` generated.

### Roadmap

#### Future improvements

 1. Update `monodoc` to do double-duty on `live=true` examples; make it so that the examples can function as tests (and thus be provably incorrect over time)
