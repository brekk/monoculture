...
describe('Sorter', () => {
  const testDeps = scenario => {
    const topo = new Topo.Sorter()
    for (const record of scenario) {
      const options =
        record.before || record.after || record.group
          ? { before: record.before, after: record.after, group: record.group }
          : null
  ...
  it('sorts dependencies', () => {
    ...
    expect(testDeps(scenario)).toEqual('0213547869')
  })
  ...
  it('sorts dependencies (manual)', () => {
    ...
    expect(topo.nodes.join('')).toEqual('')
    expect(topo.sort().join('')).toEqual('0213547869')
    expect(topo.nodes.join('')).toEqual('0213547869')
  })
  ...
  it('sorts dependencies (before as array)', () => {
    ...
    expect(testDeps(scenario)).toEqual('201')
  })
  ...
  it('sorts dependencies (after as array)', () => {
    ...
    expect(testDeps(scenario)).toEqual('120')
  })

  it('sorts dependencies (seq)', () => {
    const scenario = [{ id: '0' }, { id: '1' }, { id: '2' }, { id: '3' }]

    expect(testDeps(scenario)).toEqual('0123')
  })
  ...
  it('sorts dependencies (explicitly using after or before)', () => {
    ...
    for (let i = 0; i < groups.length; ++i) {
      ...
      scenarioAfter.push({ ...item, ...afterMod })
      scenarioBefore.push({ ...item, ...beforeMod })
    }

    fisherYates(scenarioAfter)
    expect(testDeps(scenarioAfter)).toEqual(set)

    fisherYates(scenarioBefore)
    expect(testDeps(scenarioBefore)).toEqual(set)
  })
  ...
  it('throws on circular dependency', () => {
    ...
    expect(() => {
      testDeps(scenario)
    }).toThrow('item added into group c created a dependencies error')
  })
  ...
  describe('merge()', () => {
    it('merges objects', () => {
      ...
      topo.add('6', { group: 'd' })
      topo.add('8', { before: 'd' })
      expect(topo.nodes.join('')).toEqual('02486')
      ...
      expect(other.nodes.join('')).toEqual('13579')

      topo.merge(other)
      expect(topo.nodes.join('')).toEqual('0286135479')
    })
    ...
    it('merges objects (multiple)', () => {
      ...
      expect(topo1.nodes.join('')).toEqual('0213547869')
    })
    ...
    it('throws on circular dependency', () => {
      ...
      expect(() => {
        topo.merge(other)
      }).toThrow('merge created a dependencies error')
    })
  })
})
